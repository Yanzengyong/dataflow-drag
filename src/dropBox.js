 ...
 * @LastEditTime: 2020-09-04 09:39:10
 */
...
class Box extends React.Component {
	...
	render() {
		...
		const X1 = x - (containerX - 500)

		console.log(id)
		return (
			<div onClick={() => this.props.onClick()} id={id} key={id} style={{ position: "absolute", top: y, left: X1, width: 80, height: 40, backgroundColor: '#fff000' }}>{name}</div>
		)
	}
}
    ...
    const { canDrop, isOver, connectDropTarget, dropResult } = this.props;
		...
		return connectDropTarget && connectDropTarget(
			<div className='dropBox_container'>
				...
				<div className='dropBox' id='dropBox' style={isActive ? {backgroundColor: '#ccc'} : null}>
  ...
  'box',
  {
    // 当有对应的 drag source 放在当前组件区域时，会返回一个对象，可以在 monitor.getDropResult() 中获取到
      ...
      console.log('getDifferenceFromInitialOffset===========', getDifferenceFromInitialOffset)
			...
			return {
				name: 'yzy',
				position: getSourceClientOffset ?? {}
			}
		}
  },
	(connect, monitor) => { 
